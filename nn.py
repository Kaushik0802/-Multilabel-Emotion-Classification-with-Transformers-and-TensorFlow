...
def train_advanced(
    ...
    train_path="info-557-train-dev\\train.csv",
    dev_path="info-557-train-dev\\dev.csv",
    test_path="test-in\test-in.csv",
    batch_size=32,  # Adjusted batch size
    learning_rate=2e-5,  # Slightly lower learning rate for better fine-tuning
    num_epochs=5,  # Reduced epochs for quicker training
    dropout_rate=0.3,  # Adjusted dropout to prevent overfitting
    dense_units=[256, 128],  # Two dense layers for better representation
):
    ...
    for units in dense_units:  # Add multiple dense layers
        x = tf.keras.layers.Dense(units, activation="relu")(x)
        x = tf.keras.layers.Dropout(dropout_rate)(x)  # Add dropout after each dense layer
    ...
    # Evaluate per-emotion F1 scores
    y_true = []
    ...
    # Save the transformer model and tokenizer separately
...
def predict_advanced(model_path="model.keras", transformer_path="transformer_model", input_path=r"test-in\test-in.csv"):
    # Load the tokenizer and transformer model
